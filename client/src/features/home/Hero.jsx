...
const checks = ["Document Extracted", "Face Matched", "Fraud Scan Clear", "Identity Verified"];
      ...
      <div className="relative mx-auto grid max-w-[1200px] grid-cols-1 items-center gap-16 px-6 lg:grid-cols-[1.05fr_0.95fr]">
        <div>
          ...
          <div className="mt-12 grid grid-cols-1 gap-6 border-t pt-8 sm:grid-cols-3">
            ...
            icon: Zap,
            label: "50ms Response"
          }].map(t => <div key={t.label} className="flex items-center gap-3">
        ...
        <div className="relative mx-auto w-full max-w-md">
          <div className="relative aspect-[3/4] overflow-hidden rounded-2xl border bg-gradient-to-br from-card to-[var(--color-surface)] p-6 shadow-2xl">
            ...
            <div className="mt-6 space-y-3">
              {[["Name", "Aarav Mehta"], ["DOB", "12 / 04 / 1992"], ["ID Number", "•••• •••• 4821"]].map(([k, v]) => <div key={k} className="flex items-center justify-between border-b border-border/60 pb-2 text-xs">
          ...
          <div className="mt-6 space-y-2 rounded-2xl border bg-card/60 p-4 backdrop-blur">
            {checks.map((c, i) => <div key={c} className={`flex items-center gap-3 text-sm transition-all duration-500 ${i < visible ? "opacity-100 translate-x-0" : "opacity-40 -translate-x-1"}`}>
                <span className={`inline-flex h-5 w-5 items-center justify-center rounded-full border ${i < visible ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground"}`}>
